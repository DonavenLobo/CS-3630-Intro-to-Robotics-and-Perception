...
class Map:
    ...
    def step_from_to(self, node0, node1, limit=75):
        ...
        # If the distance is less than the limit, return node1
        if dist < limit:
            return node1

        # Compute the angle of the vector using arctan2
        angle = np.arctan2((node1.x - node0.x), (node1.y - node0.y))

        # Calculate the x and y components of the desired point using the angle
        new_x = node0.x + (limit * np.cos(angle))
        new_y = node0.y + (limit * np.sin(angle))
    ...
    def node_generator(self):
        ...
        ############################################################################
        GOAL_CHANCE = 0.05  # 5% chance to return goal location
        
        if np.random.rand() < GOAL_CHANCE:
            goal_node = self.get_goals()[0]
            return Node((goal_node.x, goal_node.y))
        
        rand_node = Node((np.random.uniform(0, self.width), np.random.uniform(0, self.height)))
        while not self.is_inbound(rand_node) or self.is_inside_obstacles(rand_node):
            rand_node = Node((np.random.uniform(0, self.width), np.random.uniform(0, self.height)))
    ...
    def compute_smooth_path(self, path):
        ...
        for i in range(iterations):
            size = len(path)
            ind1 = np.random.randint(0, size - 1)
            ind2 = np.random.randint(0, size - 1)
            if ind1 == ind2:
                break
            ind_min = min((ind1, ind2)) # closer to start
            ind_max = max((ind1, ind2)) # closer to goal

            start_node = path[ind_min]
            goal_node = path[ind_max]
            if not self.is_collision_with_obstacles((start_node, goal_node)):
                goal_node.parent = start_node
                path = path[0:ind_min + 1] + path[ind_max:size + 1]
        return path
